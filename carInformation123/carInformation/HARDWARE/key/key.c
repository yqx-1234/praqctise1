...
{	
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;   		         //输入模式，默认状态是低电平
...
void KEY_Exti_Init(void)
{
	...
	EXTI_InitTypeDef   EXTI_InitStructure; //定义一个设置中断的结构体 
  	...
  	RCC_APB2PeriphClockCmd(RCC_APB2Periph_SYSCFG,ENABLE);	          //使能GPIO复用功能时钟
  	SYSCFG_EXTILineConfig(EXTI_PortSourceGPIOA,GPIO_PinSource0);    //配置KEY1-PA0中断线
	  SYSCFG_EXTILineConfig(EXTI_PortSourceGPIOA,GPIO_PinSource1);    //配置KEY2-PA1中断线
